...
export default class reseduleappointment extends Component {
  ...
  selectclinic = async (e) => {
    ...
    var availability = [];
    for (var i = 0; i < allAvailability.length; i++) {
      if (
        allAvailability[i].clinic_name === e.target.value &&
        allAvailability[i].doctor_id === doctor_id
      ) {
        availability.push(allAvailability[i]);
      }
    }
    this.setState({
      ...
      if (appointment === true) {
        ...
        if (notification === true) {
          ...
          if (newnotificationmsg === true) {
            setTimeout(() => {
              window.location.replace("/doctor/appointment");
            }, 2000);
          }
        }
      }
    ...
    return (
      <div className="dashboard">
        ...
        <div className="waitingroom">
          <div className="mt-2">
            <h5>Amend Appointments</h5>
            <p>
              <b>Modify and save appointments</b>
            </p>
          </div>
