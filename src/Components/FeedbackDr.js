...
export default class FeedbackMr extends Component {
    ...
    componentDidMount = async () => {
        ...
        var myappointment = await allappointmentdata.filter((data) => { return data.to_id === userid && data.meeting_status === "completed" && data.starto === null })
        if (myappointment.length !== 0) {
            ...
            for (var i = 0; i < user.length; i++) {
                for (var j = 0; j < myappointment.length; j++) {
                    if (user[i].userid === myappointment[j].from_id) {
